...
### 6.5.7 Milestone 13.3 gate — VM builtin stub inventory (BLOCKED)

`test/compile_encoder_wasm.py` (13.3 groundwork) compiles cognition's
nano `encoder.patched.onnx` twice — once via `tvm.compile(llvm)` for
the oracle, once via `relax.build(wasm32-wasi, system_lib=True)` for
the ported runtime. Bundle builds cleanly at 3.3 MB; the wasmtime
driver (`test/encoder_test.c`) links against the fork's static libs
with autogenerated per-kernel stubs from `test/gen_kernel_stub.py`
(100 TIR kernels). Build passes; `wasmtime run` fails at VM create:

    [encoder_test] VM create failed: Cannot find packed function
    `relax.run.shape_to_tensor` from module.

`ex.stats()` on the compiled executable lists 88 globals; the
`vm.builtin.*` / `relax.run.*` subset (all other globals are TIR
kernels lib0.o exports and the driver registers):

    vm.builtin.check_tensor_info    STUB   (returns 0; type-check no-op is arguably fine)
    vm.builtin.match_shape          STUB   (returns 0; may need to bind shape vars — audit)
    vm.builtin.shape_of             STUB   (returns 0; encoder callers get uninit register)
    vm.builtin.alloc_storage        REAL
    vm.builtin.alloc_tensor         REAL
    vm.builtin.null_value           SPECIAL (pf=NULL; handled in relax_vm.c)
    vm.builtin.reshape              REAL   (13.2b flagged pre-fix wire tag)
    vm.builtin.make_tuple           STUB   (returns 0; encoder returns 3-tuple)
    relax.run.shape_to_tensor       MISSING (not registered anywhere)

This is a NEW class of drift beyond 13.2b's audit items — the audit
called out three known no-op stubs (`MakeClosure`, `InvokeClosure`,
`CallTIRDyn`) as "encoder shouldn't hit these", but did NOT catch that
the majority of `vm_builtin.c`'s registered functions are also stubs.
Encoder crosses at least four of them (`ShapeOf`, `MakeShape`,
`MakeTuple`, `TupleGetItem`) plus needs one new global registration
(`relax.run.shape_to_tensor` — a rank-1 int64 tensor of the given
shape's dim values).

Per the M13 plan §4 13.3 stop condition ("If encoder trips a
pre-existing stub: STOP. This is plan-territory (may need 13.3b for
stub implementation)"), 13.3 halts here. Scaffolding (compile script,
wasm driver, CMake target, kernel-stub generator) is landed for
13.3b to consume. Estimated 13.3b scope: ~200-300 LoC across
`vm_builtin.c` + one new packed-function registration site, plus
a rerun of the correctness diff.

Commit: `7c16a55` (scaffolding).

### 6.5.8 Milestone 13.4 gate — VITS decoder passes at 1e-4

The decoder-gate scaffolding (`test/compile_decoder_wasm.py`,
`test/decoder_test.c`, `test/decoder_verify.py`, plus the CMake
target) compiles cognition's nano `decoder.onnx` twice — once via
`tvm.compile(llvm)` for the native oracle, once via `relax.build`
with the wasm32-wasi target for the driver — feeds both the same
deterministic `z_p` [1,128,1024] and `y_mask` [1,1,1024] input
bytes, and diffs the two float32 waveforms element-wise.

    waveform: max_abs_diff = 9.76e-07  PASS  (atol=1e-4)

No new VM builtins were needed (decoder's builtin surface is a
subset of the encoder's — `shape_of`, `make_tuple`, `tuple_getitem`,
`match_shape`, `check_tensor_info`, `reshape`, `alloc_storage`,
`alloc_tensor`; the decoder's own `ConvTranspose` and upsampling
compile down to plain TIR kernels registered exactly like the
encoder's). No new tag-branching bugs surfaced under the
`vm.builtin.` name-prefix check applied in 13.3c. No `MakeClosure`
/ `InvokeClosure` / `CallTIRDyn` stubs triggered — the decoder is
also a static Relax graph.

92 TIR kernels emitted by lib0.o; kernel-registration stub
autogenerated by `test/gen_kernel_stub.py` verbatim, no changes
needed.

Closes M13.4 decoder-gate correctness objective. The ConvTranspose
concern from §8 of the plan did not materialise as a numeric drift
